...
const std::string rate="2200000bps";//72.2Mbps (For far away nodes, the rate should be decreasesd, i.e. ~300Kbps)
static int nWifi=11;
...
int main (int argc, char *argv[])
{
  ...
  int nSinks=1; 
  double txp=7.5;//in dBm, the communication range radius will be 138.3 for 7.5 dBm
  double totalTime=30;
  uint32_t mobilityModel=5;//1-RWP, 2-GaussMarkov 3-2D RW 4-FixPos 5-FixAcceleration 
  ...
  for (int i = 0; i < nSinks; i++)
     {        
        ...
        //setupConnectionSocket(0,2,totalTime,port,adhocInterfaces,adhocNodes);
     }
setupConnectionOnOff(0,4,totalTime,port,adhocInterfaces,adhocNodes);
...
void ReceivePacket (Ptr<Socket> socket)
 {
   ...
   packetsReceived++;
   currentRxBytes+=packet->GetSize();
   
   std::cout<<packetsReceived<<" packet received, size: "<< packet->GetSize()<<std::endl;
   ...
   if (packet->RemovePacketTag (ttlTag))
     ...
     if(currentRxBytes>=totalTxBytes)
       {
          //socket->Close();
          std::cout<<"File received at: "<<Simulator::Now ().GetSeconds ()<<std::endl;
        }
...
{ 
  //while (currentTxBytes < totalTxBytes && localSocket->GetTxAvailable () > 0) 
    while (currentRxBytes<totalTxBytes && localSocket->GetTxAvailable () > 0)
    {     
      uint32_t left = totalTxBytes - currentRxBytes;
      //uint32_t dataOffset = currentTxBytes % writeSize;
      //uint32_t toWrite = writeSize - dataOffset;
      uint32_t toWrite = std::min (writeSize, left);
      toWrite = std::min (toWrite, localSocket->GetTxAvailable ());
      int amountSent = localSocket->Send (&data[writeSize], toWrite, 0);
      if(amountSent < 0)
        {  
          return;  
        }
 ...
 /*switch (mobilityModel)
    ...
    case 3:      
      mobilityAdhoc.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
                              ...
                              "Bounds", RectangleValue (Rectangle (0, X, 0, Y)));
       break;
    case 4://Important note:: It seems that "ConstantPositionMobilityModel" is sensitive to switch case, it does not work properly when positioned in the switch case, but the code works perfectly out of switch-case;
	...
	Vector node4_Position(400.0, 0.0, 0.0);	
	...
	myListPositionAllocator.Add(node4_Position);
	
	mobilityAdhoc.SetPositionAllocator(&myListPositionAllocator);
	break;
   case 5://Important note:: It seems that "ConstantPositionMobilityModel" is sensitive to switch case, it does not work properly when positioned in the switch case, but the code works perfectly out of switch-case;*/
     mobilityAdhoc.SetMobilityModel ("ns3::ConstantAccelerationMobilityModel");
     mobilityAdhoc.Install (adhocNodes);   
        Vector node1_Position(0.0, 0.0, 0.0);        
	Vector node2_Position(100.0, 0.0,0.0);
	Vector node3_Position(200.0, 0.0, 0.0);
	Vector node4_Position(300, 0.0, 0.0);
	Vector node5_Position(400.0, 0.0, 0.0);        
	Vector node6_Position(500, 0.0,0.0);
	Vector node7_Position(600.0, 0.0, 0.0);
	Vector node8_Position(700, 0.0, 0.0);
	Vector node9_Position(800.0, 0.0, 0.0);        
	Vector node10_Position(900, 0.0,0.0);
	Vector node11_Position(1000.0, 0.0, 0.0);

	Vector node1_Vel(0.0, 0.0, 0.0);
	Vector node2_Vel(0.0, 0.0,0.0);
	Vector node3_Vel(0.0, 0.0, 0.0);
	Vector node4_Vel(0.0, 0.0, 0.0);
	
	Ptr<ConstantAccelerationMobilityModel> m[nWifi];
	for(int i=0;i<nWifi;i++){
	   m[i]=DynamicCast<ConstantAccelerationMobilityModel>(adhocNodes.Get(i)->GetObject<MobilityModel>());
	   }
	m[0]->SetPosition(node1_Position);
	m[1]->SetPosition(node2_Position);
	m[2]->SetPosition(node3_Position);
	m[3]->SetPosition(node4_Position);
	m[4]->SetPosition(node5_Position);
	m[5]->SetPosition(node6_Position);
	m[6]->SetPosition(node7_Position);
	m[7]->SetPosition(node8_Position);
	m[8]->SetPosition(node9_Position);
	m[9]->SetPosition(node10_Position);
	m[10]->SetPosition(node11_Position);
	
	m[0]->SetVelocityAndAcceleration(Vector(0.0,0.0,0.0),node1_Vel);
	m[1]->SetVelocityAndAcceleration(Vector(0.0,0.0,0.0),node2_Vel);
	m[2]->SetVelocityAndAcceleration(Vector(0.0,0.0,0.0),node3_Vel);
	m[3]->SetVelocityAndAcceleration(Vector(0.0,0.0,0.0),node4_Vel);
...
// Create and bind the sink socket...
  ...
  localSocket->Connect (InetSocketAddress (adhocInterfaces.GetAddress (destination), port)); //connect
...
setupConnectionOnOff(int source,int destination,double totalTime,uint16_t  port,Ipv4InterfaceContainer adhocInterfaces,NodeContainer adhocNodes)
{
  //setup onoff helper
  //Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue ("ns3::TcpNewReno"));
  Config::SetDefault  ("ns3::OnOffApplication::PacketSize",StringValue ("1448"));
  Config::SetDefault ("ns3::OnOffApplication::DataRate",  StringValue (rate));
  OnOffHelper onoff1 ("ns3::TcpSocketFactory",Address ());
  onoff1.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"));
  onoff1.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0.0]"));
